...
const dataToPatch = require('./../data/data-to-patch');
const format  = require('date-fns/format');

const getDataToPatchMap =() => {
  return dataToPatch.reduce((op, data)=>{
    op[data.title] = data
    return op
  }, {})
}

const dataToPatchMap = getDataToPatchMap()
...
var generateFrontMatter = function (title, subtitle, date, slug, tags) {
  ...
  frontMatter.date = date ? format(new Date(date), 'yyyy-MM-dd'): "";
  frontMatter.slug = slug ? slug.toString() : "";
  frontMatter.keywords = tags ? tags : [];
  
  if(dataToPatchMap[frontMatter.title]){
    frontMatter.tags = dataToPatchMap[frontMatter.title].tags
    frontMatter.summary = dataToPatchMap[frontMatter.title].summary
  }
